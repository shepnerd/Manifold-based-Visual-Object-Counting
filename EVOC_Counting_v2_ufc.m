...
% folder definitions
idx = int32(randperm(totalNum));
f_idx = zeros(folders,10);
for i = 1 : folders
    f_idx(i,:) = idx(1 + 10 * (i-1) : 10 * i);
end
...
%         data.images = images;
        ...
        for i = 1 : folders
            if i~=k
                ct = ct + 1;
                trRange(1 + 10 * (ct-1) : 10 * ct) = f_idx(i,:);
            end
        end
        params.trRange = trRange;
        params.teRange = f_idx(k,:);
